...
@NamedNativeQueries({
        @NamedNativeQuery(
                ...
                query = "select floor(rand()*10000000) as id, productId, abs(sum(qty)) as qty,sum(amount) as amount from stockbalance s where s.qty<0 group by productId order by sum(amount) desc",
