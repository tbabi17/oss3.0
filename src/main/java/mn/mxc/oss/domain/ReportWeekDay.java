...
@NamedNativeQueries({
        @NamedNativeQuery(
                name = "weekDay",
                query = "INSERT INTO ReportWeekDay (userId,monMCount,monECount,monAmt,tueMCount,tueECount,tueAmt,wedMCount,wedECount,wedAmt,thuMCount,thuECount,thuAmt,friMCount,friECount,friAmt,satMCount,satECount,satAmt,sunMCount,sunECount,sunAmt,totalMCount,totalECount,totalAmt) select id,(select count(*) from customer where route=mon) as monMCount,(select count(*) from orders where weekDay(createdDate) = 0 and userId=p.id and customerId in (select customerId from customer where route=mon)) as monECount,(select ifnull(sum(amount),0) from orders where weekDay(createdDate) = 0 and userId=p.id) as monAmt,(select count(*) from customer where route=tue) as tueMCount,(select count(*) from orders where weekDay(createdDate) = 1 and userId=p.id and customerId in (select customerId from customer where route=tue)) as tueECount,(select ifnull(sum(amount),0) from orders where weekDay(createdDate) = 1 and userId=p.id) as tueAmt,(select count(*) from customer where route=wed) as wedMCount,(select count(*) from orders where weekDay(createdDate) = 2 and userId=p.id and customerId in (select customerId from customer where route=wed)) as wedECount,(select ifnull(sum(amount),0) from orders where weekDay(createdDate) = 2 and userId=p.id) as wedAmt,(select count(*) from customer where route=thu) as thuMCount,(select count(*) from orders where weekDay(createdDate) = 3 and userId=p.id and customerId in (select customerId from customer where route=thu)) as thuECount, (select ifnull(sum(amount),0) from orders where weekDay(createdDate) = 3 and userId=p.id) as thuAmt,(select count(*) from customer where route=fri) as friMCount,(select count(*) from orders where weekDay(createdDate) = 4 and userId=p.id and customerId in (select customerId from customer where route=fri)) as friECount, (select ifnull(sum(amount),0) from orders where weekDay(createdDate) = 4 and userId=p.id) as friAmt,(select count(*) from customer where route=sat) as satMCount,(select count(*) from orders where weekDay(createdDate) = 5 and userId=p.id and customerId in (select customerId from customer where route=sat)) as satECount,(select ifnull(sum(amount),0) from orders where weekDay(createdDate) = 5 and userId=p.id) as satAmt,(select count(*) from customer where route=sun) as sunMCount,(select count(*) from orders where weekDay(createdDate) = 6 and userId=p.id and customerId in (select customerId from customer where route=sun)) as sunECount,(select ifnull(sum(amount),0) from orders where weekDay(createdDate) = 6 and userId=p.id) as sunAmt,0,0,0 from user as p where userType=:userType"
        )
})
